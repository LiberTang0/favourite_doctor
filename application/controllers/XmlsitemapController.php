...
//                $i++;
	...
	$item->appendChild($text);/**/
        ////////////////////////////////////////////////
        $where = "status=1 AND fname!='' AND membership_level IN ('Silver', 'Bronze Premium')";
        $SeoUrl = new Application_Model_SeoUrl();
        $Doctor = new Application_Model_Doctor();
        ...
        if($object){
            foreach($object as $doc){
                ...
                $item->appendChild($text);
                        ...
                        $sitemap_file = "";
                        $sitemap_file = "sitemap-doctor2.xml";
                        $this->_dom->save($sitemap_file);
	...
	$item->appendChild($text);/**/
        ////////////////////////////////////////////////
        $where = "status=1 AND fname!='' AND membership_level IN ('Platinum', 'Gold')";
        $SeoUrl = new Application_Model_SeoUrl();
        $Doctor = new Application_Model_Doctor();
        $object = $Doctor->fetchAll($where, "fname ASC");
        if($object){
	...
	$item->appendChild($text);/**/
        ...
        $select = $db->query("SELECT city FROM doctors WHERE status=1 AND city!='' GROUP BY city ORDER BY city ASC");
    ...
    public function insuranceDentistAction() {
        ...
        $select = $db->select()
                ...
                ->where("id IN (SELECT DISTINCT insurance_company_id FROM insurance_plans
                        WHERE status=1 AND plan_type='d') AND status=1")
                ->order ("company ASC");
