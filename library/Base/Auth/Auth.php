...
class Base_Auth_Auth extends Zend_Auth {
    ...
    public function checkUserActiveStatus($options, $identityColumn) {
        ...
        if(isset($options['user_level_id']) && $options['user_level_id']!=''){ // check the user level
            $where = " AND user_level_id='{$options['user_level_id']}'";
        }
        $RES = $user->fetchRow("{$identityColumn}='{$options['email']}' AND status ='active' {$where}");
        ...
        if ($RES != false) { // if user is not blocked
            ...
            /*---------------- start set doctor/patient id in session -------------------*/
            if($RES->getUserLevelId()==2){
                $Doctor = new Application_Model_Doctor();
                $docObject = $Doctor->fetchRow("user_id='{$RES->getId()}'");
                if($docObject){
                    $usersNs->doctorId = $docObject->getId();
                }
            }elseif($RES->getUserLevelId()==3){
                $Patient = new Application_Model_Patient();
                $patObject = $Patient->fetchRow("user_id='{$RES->getId()}'");
                if($patObject){
                    $usersNs->patientId = $patObject->getId();
                }
            }
            /*---------------- end set doctor/patient id in session -------------------*/
