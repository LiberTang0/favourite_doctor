...
//                    $CatResultArray [$value->did] += 10; // commented by Developer 12 Feb
...
//                $value = mysql_real_escape_string($value);
            ...
            if ($searchtype == "PartialLang") {
//                $POINT = $matchPoint == false ? 7 : $matchPoint;
...
//                $POINT = $matchPoint == false ? 5 : $matchPoint;
...
//                $POINT = $matchPoint == false ? 10 : $matchPoint;
...
//        if(isset($fieldswithvalues['city'])){
        ...
        $where = " AND membership_level IN ({$this->_membershipLevel}) {$whr} {$zipWhere} ";
        $Doctor = new Application_Model_Doctor();
        $docObject = $Doctor->fetchAll("$searchwhereStr AND status='1' $where", "fname");
...
//        $cityname = mysql_real_escape_string($cityname);
        ...
        if(isset($stateArray['category']) && $stateArray['category']!=''){
            $queryState = "SELECT d.`state` FROM `doctors` d, `doctor_categories` dc
                        ...
                        AND (d.`city` = '$cityname' OR d.`area` = '$cityname') AND d.`status`=1 AND dc.category_id='{$stateArray['category']}'";
        }else{
            $queryState = "SELECT `state` FROM `doctors`
                            WHERE membership_level IN ({$this->_membershipLevel})
                            AND (`city` = '$cityname' OR `area` = '$cityname') AND `status`=1";
        }
			//die($queryState);
...
//                        $AllResultArray [$item_id] += $matchpoint + 30 - $delMatchpoint; // $delMatchpoint added by Developer
                        $AllResultArray [$item_id] += $matchpoint + $bonus - $delMatchpoint; // $delMatchpoint added by Developer
...
//	$query = "SELECT DISTINCT z.cityname FROM zipcodesusa z, doctors d WHERE d.city=z.cityname AND  z.cityname = '$city'";
	...
	switch ($rs->membershipLevel)
            {
                case "Platinum":
                //$POINT=250; //Developer 8 Feb 2011
...
//                        'escondido'=>'92029',
		...
		$db = Zend_Registry::get('db');
		$query = "Select zipcode from neighborhood where city='".$key."'";
		$select = $db->query($query);
        $rs_states = $select->fetchAll();
		foreach($rs_states as $rs_state) {
			return $rs_state->zipcode;
		}
		return "";
